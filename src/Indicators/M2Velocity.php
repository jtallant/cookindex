<?php

namespace DDI\Indicators;

use DDI\IndicatorResult;
use DDI\TimeSeries;

class M2Velocity
{
    private const HISTORICAL_RATE = -0.03;

    public function compute(TimeSeries $m2v): IndicatorResult
    {
        $latest = $m2v->latest();
        $yearAgo = $m2v->yearAgo();

        if ($latest === null || $yearAgo === null) {
            return new IndicatorResult(0.0, 'N/A');
        }

        $yoyChange = $latest['value'] - $yearAgo['value'];
        $acceleration = $yoyChange - self::HISTORICAL_RATE;

        $signal = 0.0;
        if ($acceleration < 0) {
            $excess = abs($acceleration);
            if ($excess <= 0.02) $signal = 0.2;
            elseif ($excess <= 0.05) $signal = 0.4;
            elseif ($excess <= 0.10) $signal = 0.7;
            else $signal = 1.0;
        }

        $detail = sprintf(
            '%.3f → %.3f (YoY: %+.3f)',
            $yearAgo['value'], $latest['value'], $yoyChange
        );

        return new IndicatorResult($signal, $detail);
    }
}
